...
def evaluate_model_box_mask(model, dataset_yaml_path, test_loader, device, conf=0.25, iou=0.3):
    """
    Evaluate YOLO model using metrics similar to the UNet evaluation.
    ...
    # Run YOLO validation on the dataset
    val_results = model.val(data=dataset_yaml_path, conf=conf, iou=iou, split='test')
...
def evaluate_model_box(model, dataset_yaml_path, test_loader, device, conf=0.25, iou=0.3):
    """
    Evaluate YOLO model using metrics similar to the UNet evaluation.
