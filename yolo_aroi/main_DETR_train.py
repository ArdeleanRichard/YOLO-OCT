...
from constants import dataset_yaml_path, yolo_pretrained_model, yolo_model_config, yolo_model_path, yolo_model_name, detr_pretrained_model, detr_model_name, detr_model_path
...
if __name__ == '__main__':
    ...
    model = RTDETR(detr_pretrained_model)
    ...
    results = model.train(
        ...
        name=detr_model_name,  # experiment name
    )

    print("Training completed.")
    # Save the current state of the model to a file.
    model.save(detr_model_path)

    evaluation_results = model.val(
        data=dataset_yaml_path,
        imgsz=640,
        project="runs",
        name=detr_model_name,
    )
