...
from constants import dataset_yaml_path, data_root, yoloe_model_path, yoloworld_model_path
import torch
from torch.utils.data import DataLoader
from yolo_aroi.f_testing import YOLOtoSegmentationDataset, evaluate_model, evaluate_model_box

if __name__ == "__main__":
    model = YOLOWorld(yoloworld_model_path)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    test_dataset = YOLOtoSegmentationDataset("../" + data_root + "saved_classes_all_sub_only_remap/", image_size=512, split="test")
    ...
    metrics = evaluate_model_box(model, dataset_yaml_path, test_loader, device)
